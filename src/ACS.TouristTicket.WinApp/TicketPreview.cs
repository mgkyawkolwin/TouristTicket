...
namespace ACS.TouristTicket.WinApp
{
    public partial class TicketPreview : Form
    {
        ...
        {
            CheckPrintingCode checkCode = formFactory.CreateCheckCode();
            checkCode.ShowDialog();

            if (!checkCode._b)
            {
                MessageBox.Show("Invalid printing approval code !", "Invalid Code", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }

            if (MessageBox.Show("Are you sure to print this TicketID collections ?\n\nTicket Total : " + ticketCollection.Count().ToString()  +
                "\n\nTo advoid illegal printing and duplication of TicketID, you may not allowed to re-print this TicketID collections again." +
                "\nSelect YES to continue printing or NO to cancel printing. \n\n Continue Printing ???", "Print", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == System.Windows.Forms.DialogResult.Yes)
            {
                webBrowser1.Print();
                for (int i = 0; i < ticketCollection.Count(); i++)
                {
                    new ACSFCTicket().Update_TicketStatus(ticketCollection.ElementAt(i).PKID, "PRINTED", 1, DateTime.Now);
                }
                TicketList ticketList = formFactory.CreateTicketList();
                ticketList.isPrinted = true;

                new ACSFCTicket().DeletePrintingCode();
            }
        }
